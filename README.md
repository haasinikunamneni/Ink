...
I took inspiration from the dynamic nature of bullet journalling leaning heavy functionality which was the main goal.
...
## If Apple Notes archiving ever fails

The app now also writes a plain-text backup of every archived day to:
```
~/Library/Application Support/BulletJournal/Archive/YYYY-MM-DD.txt
```
This happens **every time a day rolls over, regardless of whether the
Apple Notes automation permission was granted** — so even if Notes
integration silently fails (the most likely cause being that macOS
never got permission to automate Notes.app, or it was revoked), nothing
is lost. Two related fixes went in together:

- **Tasks you never touched — left as a plain `○` with no action taken
  — used to be silently dropped from the archive entirely.** They're
  now included under a "Not Completed" section, both in the Notes note
  and the local backup file.
- If you ever suspect Notes archiving isn't working, check **System
  Settings → Privacy & Security → Automation → Bullet Journal** and
  make sure "Notes" is checked. The local `.txt` backup will have your
  data either way.
